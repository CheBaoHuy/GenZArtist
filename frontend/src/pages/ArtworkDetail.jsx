...
export default function ArtworkDetail() {
    ...
    const [myStatus, setMyStatus]   = useState(null); // { purchased, alreadyReviewed, myReview }
    ...
    /* ── Fetch purchase / review eligibility ─────── */
    useEffect(() => {
        if (!token) { setMyStatus(null); return; }
        setStatusLoading(true);
        fetch(`${API}/reviews/product/${id}/my-status`, {
            ...
            .then(res => {
                if (res.status === 'success') {
                    ...
                    if (res.data.myReview) {
                        setMyRating(res.data.myReview.rating || 0);
    ...
    const handleSubmitReview = async (e) => {
        ...
        try {
            ...
            if (data.status === 'success') {
                setRvSuccess(myStatus?.alreadyReviewed ? 'Đã cập nhật đánh giá của bạn! 🎉' : 'Đánh giá của bạn đã được ghi nhận! 🎉');
                setMyStatus(prev => ({ ...(prev || {}), purchased: true, alreadyReviewed: true, myReview: data.data }));
                  ...
                  <span style={{ color: 'rgba(255,255,255,.5)', fontSize: '.78rem', marginLeft: '.3rem' }}>
                    ...
                    <div className="reviews-section">
                        ...
                        {!token ? (
                            ...
                            <p className="rv-loading">Đang kiểm tra...</p>
                        ) : myStatus && !myStatus.purchased ? (
                            <div className="review-login-prompt">
                                <span>Bạn cần <strong>mua và hoàn tất thanh toán</strong> sản phẩm này để có thể đánh giá.</span>
                            </div>
                        ) : (
                                ...
                                <div className="review-form-inner">
                                    ...
                                    <div className="review-inputs">
                                        ...
                                        <button type="submit" className="review-submit-btn" disabled={rvSubmitting}>
                                            {rvSubmitting ? 'Đang gửi…' : (myStatus?.alreadyReviewed ? 'Cập nhật đánh giá' : 'Gửi đánh giá')}
                                        </button>
                                    </div>
                                </div>
                            </form>
